  ...
  const fetchData = async () => {
    try {
      ...
      if (statsResponse.data.success) {
        ...
        if (data.bulk) {
          const totalBulkOps =
            data.bulk.lastMonth.operations + data.bulk.downloads.operations;
          const totalBulkCerts =
            data.bulk.lastMonth.certificates + data.bulk.downloads.certificates;
          setBulkStats({
        ...
        {activityLog.length === 0 ? (
          <div className="text-center py-12 text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm mt-2">
              Activity will appear here once certificates are created
            </p>
            <button
              onClick={fetchData}
            ...
            {activityLog.map((log, index) => {
              ...
              return (
                ...
                >
                  {/* Icon */}
                  <div
                    className={`${colors.icon} p-2 rounded-lg flex-shrink-0 shadow-sm`}
                  >
                    {icon}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {isBulk && log.count && (
                        <span
                          className={`${colors.badge} text-xs font-medium px-2 py-0.5 rounded-full`}
                        >
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    <div
                      className={`w-2 h-2 rounded-full ${colors.dot} shadow-lg`}
                    />
                  </div>
                </motion.div>
              );
